...
import com.eguide.yash1300.e_guide.listeners.student.StudentFetchAllProjectsListener;
...
public class StudentProjectAllFragment extends Fragment {

    Context context;
    String token;
    RecyclerView recyclerView;

    public StudentProjectAllFragment(Context context, String token) {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View rootView = inflater.inflate(R.layout.fragment_student_project_all, container, false);

        recyclerView = rootView.findViewById(R.id.student_project_unapp_rv);
        recyclerView.setLayoutManager(new LinearLayoutManager(context));

        NetworkManager.getInstance().fetchAllProjects(token, new StudentFetchAllProjectsListener() {
            @Override
            public void onSuccess(List<ProjectModel> projects) {
                StudentProjectAdapter studentProjectAdapter = new StudentProjectAdapter(context, projects, token, true);
